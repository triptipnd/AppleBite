pipeline {
    ...
    environment {
        SSH_CRED_ID = 'jenkins-ssh'         // Jenkins SSH credential id (set this in Jenkins Credentials)
        TEST_SERVER = '172.18.54.197'       // test server IP
        PROD_SERVER = "${env.PROD_SERVER}"  // set this as Jenkins global env/job parameter
    ...
    stages {
        ...
        stage('Job 2 - Install Docker via Ansible') {
            steps {
                ...
                ansible-playbook -i inventory_test install-docker.yml --ssh-extra-args='-o StrictHostKeyChecking=no'
                '''
            }
        }
...
pipeline {
    ...
    environment {
        SSH_CRED_ID   = 'jenkins-ssh'           // Jenkins SSH credential id
        TEST_SERVER   = '172.18.54.197'         // test server IP
        PROD_SERVER   = "${env.PROD_SERVER}"    // optional prod server (set in Jenkins job config)
        IMAGE_NAME    = "myphpapp:latest"
        CONTAINER_NAME = "php-webapp"
        GIT_REPO      = 'https://github.com/triptipnd/AppleBite.git'
    }
